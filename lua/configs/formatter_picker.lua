...
function M.choose_for_filetype(ft, priority_delay)
  ...
  utils.on_registry_ready(function()
    local function run()
      ...
      if ok then
        for _, c in ipairs(candidates) do
          ...
          if (info and info.command) or (info_alt and info_alt.command) or utils.list_contains(utils.get_builtins(ft, "Formatter"), c) then
      ...
      selection_ui.select(items, {
        ...
        if tool ~= "None" then
          ...
          if utils.list_contains(builtins, tool) then
    ...
    if priority_delay and priority_delay > 0 then
      vim.defer_fn(run, priority_delay)
    else
      run()
    end
  end)
...
function M.setup()
  ...
  local function check_buffer(buf)
    ...
    if saved and saved ~= "" then
      local is_builtin = utils.list_contains(utils.get_builtins(ft, "Formatter"), saved)
